...
class CleanTransformer(TransformerMixin):
    ...
    def __init__(self,
        ...
        lang="en",
            ...
            """
            self.params = {}
            self.params['fix_unicode'] = fix_unicode
            self.params['to_ascii'] = to_ascii
            self.params['lower'] = lower
            self.params['normalize_whitespace'] = normalize_whitespace
            self.params['no_line_breaks'] = no_line_breaks
            self.params['strip_lines'] = strip_lines
            self.params['keep_two_line_breaks'] = keep_two_line_breaks
            self.params['no_urls'] = no_urls
            self.params['no_emails'] = no_emails
            self.params['no_phone_numbers'] = no_phone_numbers
            self.params['no_numbers'] = no_numbers
            self.params['no_digits'] = no_digits
            self.params['no_currency_symbols'] = no_currency_symbols
            self.params['no_punct'] = no_punct
            self.params['no_emoji'] = no_emoji
            self.params['replace_with_url'] = replace_with_url
            self.params['replace_with_email'] = replace_with_email
            self.params['replace_with_phone_number'] = replace_with_phone_number
            self.params['replace_with_number'] = replace_with_number
            self.params['replace_with_digit'] = replace_with_digit
            self.params['replace_with_currency_symbol'] = replace_with_currency_symbol
            self.params['replace_with_punct'] = replace_with_punct
    ...
    def transform(self, X: Union[List[str], pd.Series]) -> Union[List[str], pd.Series]:
        ...
        """
        assert isinstance(X, list) or isinstance(X, pd.Series)
        if len(X) > 0:
            assert type(X[0]) == str
        if isinstance(X, pd.Series):
            return X.apply(lambda text: clean(text, **self.params))
        else:
            return list(map(lambda text: clean(text, **self.params), X))
